...
const filterQuestionsFromQuizz = (quizId) => {
    ...
    return questions.filter((question) => parseInt(question.quizId) === parsedId)
}
...
const getQuestionFromQuiz = (quizId, questionId) => {
    ...
    if (parseInt(question.quizId) !== quizIdInt) throw new NotFoundError(`${question.name} id=${questionId} was not found for ${quiz.name} id=${quiz.id} : not found`)
    return question
}
